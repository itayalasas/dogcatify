  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      ...
      for (const table of tablesToClean) {
        try {
          console.log(`Cleaning references from ${table}...`);
          
          // Different deletion strategies based on table structure
          if (table === 'pets' || table === 'pet_albums' || table === 'pet_behavior' || table === 'pet_health' || table === 'medical_alerts' || table === 'medical_history_tokens') {
            // These tables reference user through owner_id or user_id
            await supabase.from(table).delete().eq('owner_id', userId);
            await supabase.from(table).delete().eq('user_id', userId);
          } else if (table === 'bookings' || table === 'orders' || table === 'service_reviews') {
            // These tables reference user through customer_id
            await supabase.from(table).delete().eq('customer_id', userId);
          } else if (table === 'chat_conversations' || table === 'adoption_chats') {
            // These tables reference user through user_id
            await supabase.from(table).delete().eq('user_id', userId);
          } else if (table === 'chat_messages' || table === 'adoption_messages') {
            // These tables reference user through sender_id
            await supabase.from(table).delete().eq('sender_id', userId);
          } else if (table === 'promotions' || table === 'places' || table === 'promotion_billing') {
            // These tables reference user through created_by
            await supabase.from(table).delete().eq('created_by', userId);
          } else {
            // Default: try common user reference columns
            await supabase.from(table).delete().eq('user_id', userId);
          ...
          console.warn(`Warning: Could not clean ${table}:`, cleanError.message);
          // Continue with other tables even if one fails
        }
      }
      
      console.log('All database references cleaned, now attempting auth deletion...');
    } catch (cleanupError) {
