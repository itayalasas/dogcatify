...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { recordData, token }: SaveRecordRequest = await req.json();
        ...
        if (alertDate > new Date()) {
          await supabase
            ...
            .insert({
              ...
              priority: recordData.name?.toLowerCase().includes('dhpp') || 
                       recordData.name?.toLowerCase().includes('rabia') ? 'high' : 'medium',
              status: 'pending'
            });
